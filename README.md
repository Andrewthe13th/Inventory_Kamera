...
A fan-made Genshin Impact tool that scans your Characters, Weapons, Artifacts, Materials, and Character Development items in your Inventory using the OCR technique.

This scanner exports in `.GOOD`, a JSON-based  export format, which allows you to use it with compatible online Genshin Impact tools. These tools include artifact optimizing tools including [Genshin Optimizer](https://frzyc.github.io/genshin-optimizer/#/), [SEELIE.me](https://seelie.me/), and [Aspirine's Genshin Impact Calculator](https://genshin.aspirine.su/).
...
For Inventory Kamera, you will need the **following things installed on your device**:
...
3. Go to `Settings` (Cog Icon ⚙) and set it to these settings:
   - Under `Languages`, set the *Game Language* to English.
   - Under `Graphics`, set the *Display Mode* to *any windowed resolution* that is *16:9 or 16:10 ratio*. Make sure that the entire game window is visible on the screen.
   ...
   - Under `Controls`, set the *Control Type* to Keyboard.
      - If you have rebounded the inventory key (default: B) or character screen key (default: C), either revert your binding to default or set the key binding in Inventory Kamera.


## How to Configure Inventory Kamera
Before starting the scanner, you can (optionally) choose to edit the following configurations:

- Configure which category (Weapons, Artifacts, Characters, Items) you want to be scanned.
- Configure the minimum weapon and artifact rarity to be scanned.
- Set the scanner delays to *slow* or *slower* if you have a weaker PC.
- Set the file export destination in the File Directory.


## How to Run Inventory Kamera
1. Open [Paimon menu](https://genshin-impact.fandom.com/wiki/Paimon_Menu) (Default shortcut: `ESC`) and leave it open.
2. Open Inventory Kamera and configure the settings as you wish, or else leave them as default.
3. Start the scan by clicking on the 'Scan Genshin' button. Leave it to scan and do not use your mouse or keyboard in the meantime, as it will interrupt the scanning process.
4. Once the scan is finished, you can find the exported scan file in the GenshinData folder within the same folder as the application or in the folder that you have set the exports to.

> **Warning**<br>
> While scanning, **do not use your mouse or keyboard**. The scanner uses mouse and keyboard inputs to automate artifact scanning.
>
> If you would like to terminate the scan, press the `ENTER` button at any time. However, the application will not output any scanned results. You may press the 'Export Scanned Data' button to force an export of data that was collected in the most recent scan (complete or incomplete). 


## Report an Issue
If you run into a problem with our scanner (e.g. bug, app crash, invalid export format), please [report your issue here](https://github.com/Andrewthe13th/Inventory_Kamera/issues/new/choose) and try to fill it out as much as possible. The bug report, along with the evidence, will greatly speed up the bug fixing process.

> **Note**<br>
> Before submitting an issue, **please [check that there is or are no similar issue(s)](https://github.com/Andrewthe13th/Inventory_Kamera/issues?q=is%3Aissue), especially ones that are still open.**<br>
...
We would **love to have Screenshots (especially video recordings!) and Error Logs as evidence**. These can be very helpful in debugging your issue. Add it to the issue by drag-and-drop or  attaching the file to the issue template. Inventory Kamera may take screenshots under the `logging` folder divided into categories when it might have encountered an issue. Zipping up the `logging` folder is the best way to submit your logs. You may check the 'Log All Screenshots' box to force this behavior on most areas that may be cause for concern to Devs.
...
## Ask a question
General questions? Start by looking in [Inventory Kamera's Discussion forum](https://github.com/Andrewthe13th/Inventory_Kamera/discussions). If you have a question that doesn't have a thread, you may start a new [General thread](https://github.com/Andrewthe13th/Inventory_Kamera/discussions/new?category=general) or [Q&A thread](https://github.com/Andrewthe13th/Inventory_Kamera/discussions/new?category=q-a).
